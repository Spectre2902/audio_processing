...
def main():
    ...
    if uploaded_file is not None:
        ...
        if file_type == 'video':
            ...
            st.success("Audio extracted and saved.")
        else:
            st.info("Using uploaded audio file directly...")
            audio_path = uploaded_file.name
            with open(audio_path, "wb") as f:
                f.write(uploaded_file.getbuffer())
            st.success("Audio file ready for processing.")

        st.info("Normalizing audio...")
        ...
        st.info("Configure CleanVoice Processing Options:")
        ...
        if st.button("Start CleanVoice Processing"):
            st.info("Uploading to CleanVoice...")
            signed_url = upload_to_cleanvoice(cleaned_audio_path)
            st.success("Audio uploaded to CleanVoice.")

            st.info("Requesting CleanVoice processing...")
            id_value = request_cleanvoice_processing(signed_url, config)
            st.success("Processing ID received.")
            ...
            if status == 'SUCCESS':
                ...
                st.success("Processing complete! Download your file here:")
